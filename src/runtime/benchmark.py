...
def benchmark(model_path: str, amount_of_images, progress_callback=None):
    """Run benchmark for a single model.

    progress_callback: optional callable(current_index:int, total:int, interim_metrics:dict)
    """
    ...
    images = list(os.listdir(IMAGES_PATH))[:amount_of_images]
    total = len(images)
    ...
    for idx, image in enumerate(images):
        ...
        # build interim metrics
        interim = {
            'inference_time (ms)': float(np.mean(time_values)),
            'memory_usage (MiB)': float(np.mean(memory_values)),
            'cpu_temperature (C)': float(np.mean(temp_values)),
            'cpu_usage (%)': float(np.mean(cpu_usage_values)),
            'energy_consumption (W)': float(np.mean(energy_values)),
            'processed': idx + 1,
            'total': total
        }
        ...
        if progress_callback:
            try:
                progress_callback(idx + 1, total, interim)
            except Exception:
                # Don't let progress failures stop the benchmark
                pass
